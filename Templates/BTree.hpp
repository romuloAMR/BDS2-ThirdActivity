...
=======
    ...
    void searchAndPrintLevel(BTreeNode<T, ORDER>* currentNode, int key, int level) {
    if (currentNode == nullptr) {
    ...
    if (i < currentNode->size && key == currentNode->keys[i]) {
        std::cout << "Value " << key << " found at level " << level << ".\n";
        return;
    }

    // Se for uma folha, o valor não está presente
    if (currentNode->leaf) {
        std::cout << "Value " << key << " not found in the tree.\n";
        return;
    }

    // Buscar no filho apropriado, aumentando o nível
    searchAndPrintLevel(currentNode->children[i], key, level + 1);
    }
...
public:
    ...
    void searchAndPrintLevel(int key) {
        searchAndPrintLevel(root, key, 0);
    }
